...
[ArcFace](https://arxiv.org/abs/1801.07698) suggested generalization of loss function used by [SphereFace](https://arxiv.org/abs/1704.08063), [CosFace](https://arxiv.org/abs/1801.09414) and some other approaches for face recognition. The loss function of ArcFace looks like
...
A big question is how to choose $$s$$, $$m_1$$, $$m_2$$, $$m_3$$ for a particular task. An attempt to answer this question was done in the paper [AdaCos](https://arxiv.org/abs/1905.00292) in which it was suggested an approach for the loss modelling, and how the loss' hyperparameters can be chosen.

Following AdaCos lets consider just one summand of the loss function, and lets consider not the cross-entropy itself, but only the probability of embedding to be classified correctly as a function of angle between the embedding and its class center:
...
Authors of AdaCos mentioned, that during the training the angles $$\theta_{j}$$ become close to $$\frac{\pi}{2}$$ in the very first training steps. My experience shows that it is not the whole truth and we will discuss it later. For now we will assume $$\theta_{j} = \frac{\pi}{2}$$.

The problem with this function is that it is not monotonically decreasing on $$\theta$$ for some $$m_1$$ and $$m_2$$. The minimal value for this function is when $$\theta = \frac{\pi - m_2}{m_1}.$$ Let's look at the plot of the function for a particular choose of loss parameters values:
...
Even though it could not be seen on the probabilities plot, we can see on the loss plot that probabilities of embeddings to be classified correctly start growing for $$\theta\geq\frac{3\pi}{4}$$. It means that the loss will push the weights of the network in the direction where the angles between embeddings and their class centers are close to $$\pi$$.

So the further $$\arg\min_{\theta}{P}$$ from $$\pi$$ the bigger probability that some of training samples will lay in the 'wrong' part of the curve, and thus loss function will push the network parameters in the wrong direction, trying to make the angle between these samples and their class center close to $$\pi$$.

Some researchers fix it brutally. For instance, [arcface-pytorch](https://github.com/ronghuaiyang/arcface-pytorch) suggests the following fix ($$m$$ here is $$m_2$$ in my notation):
